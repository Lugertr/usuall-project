...
import { loadShop, loadShopFailure, loadShopSuccess, updateShop, updateShopFailure, updateShopSuccess } from './auth.actions';
  ...
  loadShop$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, token]) => {
        ...
        return this.authService.getShop().pipe(this.loadingBarSrv.withLoading());
      }),
      map(shop => {
        if (shop) {
